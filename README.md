# YADROTestTask
![Ubuntu Build and test](https://github.com/idigitallegacy/YADROTestTask/actions/workflows/ubuntu-build-test.yml/badge.svg)

![Windows Build and test](https://github.com/idigitallegacy/YADROTestTask/actions/workflows/windows-build-test.yml/badge.svg)
...
<Path_to_nmake_executable>\nmake.exe "<Path_to_unpacked_archive>\YADROTestTask-dev"
```
...
Если вы на 100% уверены, что это не самореференсная ячейка, попробуйте увеличьте константу `MAX_RECURSION_DEPTH` внутри файла `cfg.h` (находится в корне репозитория). По умолчанию, вычисления производятся до 300-й запрошенной ячейки. Если это не помогло, то ошибка в кроется в логике определения самореференсных ячеек.
