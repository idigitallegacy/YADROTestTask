...
./TestTaskTest
```
Учтите, что необходимо изменить константу `TEST_DATA_PATH` в файле `cfg.h` (в корне репозитория) на путь, соответствующий директории `TestData` репозитория. Эта константа необходима только для запуска GTest-приложения и не влияет на работу основного консольного приложения.
### Windows
#### Main
Из той же директории, в которой выполнялась команда `make` выполните:
```shell
